...
import streamIoClient from '../../stream/instance.ts';
...
const Home = () => {
  ...
  const onLogin = useCallback(
    async (userId: string, token: string) => {
      if (streamIoClient.user?.id) {
        await ChatController.logout();
      }
      console.log('Loggin in', userId);
      ChatController.loginUser(userId, token)
        .then(res => {
          console.log('res==>', res);
          dispatch(
            setUser({
  ...
  useEffect(() => {
    if (curUserId) {
      console.log('Cur User is==>', curUserId);
      const user = USERS.find(user => user.userid === curUserId);
      if (user) {
        ChatController.loginUser(user.userid, user.token)
          .then(res => {
            console.log('res==>', res);
          })
          .catch(er => {
